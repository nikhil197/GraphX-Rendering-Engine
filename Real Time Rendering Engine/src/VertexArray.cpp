...
namespace engine
{
	namespace arrays
	{
		...
		void VertexArray::AddBuffer(buffers::VertexBuffer& vbo, buffers::VertexBufferLayout& layout)
		{
			...
			for (unsigned int i = 0; i < elements.size(); i++)
			{
				...
				//increase the offset
				offset += element.count * buffers::BufferLayoutElement::GetSizeOfType(element.type);
			}
		}
