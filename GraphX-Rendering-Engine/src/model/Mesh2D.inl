...
{
	template<typename T>
	Mesh2D<T>::Mesh2D(const gm::Vector3& Pos, const gm::Vector3& Rotation, const gm::Vector2& Scale, Texture * texture, Shader& shader, std::vector<T>& vertices, std::vector<unsigned int>& indices)
	...
	template<typename T>
	gm::Matrix4 Mesh2D<T>::GetTransformationMatrix() const
	{
