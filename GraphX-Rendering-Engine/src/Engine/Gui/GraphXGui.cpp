...
namespace GraphX
{
	...
	void GraphXGui::DetailsWindow(const Ref<Mesh3D>& mesh, const std::string& Name)
	{
		...
		{
			ShowDetailsWindow = mesh->bShowDetails;
			ImGui::Begin(Name.c_str(), &ShowDetailsWindow);
			ImGui::Spacing();
			...
			ImGui::EndChild();
			ImGui::End();
			mesh->bShowDetails = ShowDetailsWindow;
		}
	}
	...
	void GraphXGui::DetailsWindow(const Ref<Mesh2D>& mesh)
	{
		...
		{
			ShowDetailsWindow = mesh->bShowDetails;
			ImGui::Begin("Details", &ShowDetailsWindow);
	...
	{
		static bool ShowLightProperties = true;
		if (light->bShowDetails)
		{
			ShowLightProperties = light->bShowDetails;
			ImGui::Begin("Lighting", &ShowLightProperties);
			...
			ImGui::End();
			light->bShowDetails = ShowLightProperties;
		}
	}
	...
	void GraphXGui::TerrainDetails(const Ref<Terrain>& terrain)
	{
		...
		if (ShowTerrainDetailsWindow)
		{
			...
			{
				terrain->GetMesh()->UpdateModelMatrix(true);
			}
