...
namespace GraphX
{
	PointLight::PointLight(const GM::Vector3& Pos, const GM::Vector4& Col, float Intensity, const GM::Vector3& AttenuationFactors)
		: Light(Col, GM::ProjectionMatrix::Ortho(-10.0f, 10.0f, -10.0f, 10.0f, 1.0f, 1000.0f), Intensity), Position(Pos), m_AttenuationFactors(AttenuationFactors)
	{
	}
	...
	void PointLight::Update(float DeltaTime)
	{
		...
		m_LightShadowInfo->LightViewProjMat = m_LightShadowInfo->LightProjMat * GM::ViewMatrix::LookAt(Position, GM::Vector3(0.0f), GM::Vector3::YAxis);
	}
