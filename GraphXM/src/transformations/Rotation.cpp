...
namespace GraphXMaths
{
	...
	{
		M[0][0] =  (float)Cos(Value);
		M[0][1] = -(float)Sin(Value);
		M[1][0] =  (float)Sin(Value);
		M[1][1] =  (float)Cos(Value);
	}
		...
		m_Angles(Angles)
	{
		...
		resultX[1][1] =  (float)Cos(Angles.x);
		resultX[2][2] =  (float)Cos(Angles.x);
		resultX[1][2] = -(float)Sin(Angles.x);
		resultX[2][1] =  (float)Sin(Angles.x);

		// Rotating about y - axis
		resultY[0][0] =  (float)Cos(Angles.y);
		resultY[0][2] =  (float)Sin(Angles.y);
		resultY[2][2] =  (float)Cos(Angles.y);
		resultY[2][0] = -(float)Sin(Angles.y);

		// Rotating about z - axis
		resultZ[0][0] =  (float)Cos(Angles.z);
		resultZ[0][1] = -(float)Sin(Angles.z);
		resultZ[1][0] =  (float)Sin(Angles.z);
		resultZ[1][1] =  (float)Cos(Angles.z);
	...
	Rotation::Rotation(float Angle, const Vector3& Axis)
	{
		...
		float Roll = MagXY == 0 ? 0.0f : (float)CosInverse(Vector3::DotProduct(ProjXY, Vector3::XAxis) / MagXY);
		float Yaw  = MagXZ == 0 ? 0.0f : (float)CosInverse(Vector3::DotProduct(ProjXZ, Vector3::XAxis) / MagXZ);
